...
const MODE_COLORS = { work: 'text-primary-500', shortBreak: 'text-success-500', longBreak: 'text-accent-500' }
  ...
  return (
    <PageWrapper>
      <div className="flex items-center justify-between mb-8">
        ...
        <Button variant="ghost" size="icon" onClick={() => setSettingsOpen(true)}><Settings size={18} /></Button>
      </div>
        ...
        {Object.entries(MODE_LABELS).map(([mode, label]) => (
          <button key={mode} onClick={() => timer.setMode(mode)}
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
              timer.mode === mode ? 'bg-white dark:bg-surface-700 text-ink-900 dark:text-ink-100 shadow-sm' : 'text-ink-400 hover:text-ink-700 dark:hover:text-ink-100'
            }`}>{label}</button>
        ))}
      </div>
      <div className="flex flex-col items-center mb-10">
        <div className="relative">
          ...
          <div className="absolute inset-0 flex flex-col items-center justify-center">
            <span className={`text-5xl font-light timer-nums ${MODE_COLORS[timer.mode]}`}>{formatTime(timer.secondsLeft)}</span>
            <span className="text-sm text-ink-400 mt-1">{MODE_LABELS[timer.mode]}</span>
          </div>
        </div>
        <p className="text-xs text-ink-400 mt-4">Session {timer.sessionCount + 1} of {timer.settings.longBreakInterval}</p>
      </div>
      ...
      <div className="flex flex-col items-center gap-3 mb-6">
        <div className="flex items-center gap-2">
          <Volume2 size={13} className="text-ink-400" />
          <span className="text-xs text-ink-400 font-medium">Ambient sound</span>
        </div>
        <div className="flex gap-2">
          {SOUNDS.filter(s => s.id !== 'none').map(s => (
            <button key={s.id} onClick={() => play(active === s.id ? 'none' : s.id)}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-medium border transition-all ${
                active === s.id ? 'bg-primary-50 dark:bg-primary-700/20 border-primary-400 text-primary-600 dark:text-primary-300'
                : 'border-surface-200 dark:border-surface-700 text-ink-400 hover:border-surface-300 dark:hover:border-surface-600'
              }`}>{s.emoji} {s.label}</button>
          ))}
        </div>
        {active !== 'none' && (
          <div className="flex items-center gap-2 w-40">
            <span className="text-xs text-ink-400">Vol</span>
            <input type="range" min="0" max="1" step="0.05" value={volume} onChange={e => setVolume(Number(e.target.value))} className="flex-1 accent-primary-500" />
          </div>
        )}
      </div>
      ...
      <SettingsModal open={settingsOpen} onClose={() => setSettingsOpen(false)} settings={timer.settings} onSave={timer.updateSettings} />
    </PageWrapper>
  )
}
