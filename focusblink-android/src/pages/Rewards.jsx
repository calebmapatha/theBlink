  ...
  return (
    <PageWrapper>
      ...
      <Card glow className="p-6 mb-4 text-center">
        <div className="w-16 h-16 rounded-2xl bg-primary-500 flex items-center justify-center mx-auto mb-3"><Trophy size={28} className="text-white" /></div>
        ...
        <Card className="p-5 mb-6">
          ...
          <div className="h-3 rounded-full bg-surface-100 dark:bg-surface-700 overflow-hidden">
            <motion.div className="h-full rounded-full bg-gradient-to-r from-primary-500 to-accent-500"
              animate={{ width: `${Math.min(100, progress)}%` }} transition={{ duration: 0.8, ease: 'easeOut' }} />
          </div>
      ...
      <div>
        ...
        {history.length === 0 ? (
          <div className="py-8 text-center"><Zap size={32} className="text-ink-400 mx-auto mb-2 opacity-40" /><p className="text-sm text-ink-400">Complete tasks, habits, and focus sessions to earn XP!</p></div>
        ) : (
          <div className="space-y-2">
            {history.slice(0, 20).map((item, i) => (
              <motion.div key={item.id} initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.03 }}
                ...
                <span className="text-sm font-semibold text-success-600 dark:text-success-400 timer-nums flex-shrink-0">+{item.points}</span>
              </motion.div>
            ))}
          </div>
        )}
