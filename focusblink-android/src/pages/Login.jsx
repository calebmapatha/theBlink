      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }} className="w-full max-w-sm">
        ...
        <div className="bg-surface-800 border border-surface-700 rounded-2xl p-6 shadow-card-dark">
            ...
            {['signin', 'signup'].map(m => (
              <button key={m} onClick={() => setMode(m)}
                className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
                  mode === m ? 'bg-surface-700 text-ink-100 shadow-sm' : 'text-ink-400 hover:text-ink-100'
                }`}>
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="flex-1 h-px bg-surface-700" /><span className="text-xs text-ink-400">or</span><div className="flex-1 h-px bg-surface-700" />
          </div>
          <form onSubmit={handleEmail} className="space-y-3">
            <div className="relative">
              <Mail size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-ink-400" />
              <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="Email" required
                className="w-full pl-9 pr-4 py-2.5 rounded-xl border border-surface-600 bg-surface-900 text-ink-100 text-sm placeholder-ink-400 focus:outline-none focus:ring-2 focus:ring-primary-500" />
            </div>
            <div className="relative">
              ...
              <input type={showPw ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="Password" required
                className="w-full pl-9 pr-10 py-2.5 rounded-xl border border-surface-600 bg-surface-900 text-ink-100 text-sm placeholder-ink-400 focus:outline-none focus:ring-2 focus:ring-primary-500" />
              <button type="button" onClick={() => setShowPw(s => !s)} className="absolute right-3 top-1/2 -translate-y-1/2 text-ink-400 hover:text-ink-100">
            ...
            <AnimatePresence>
              {authError && (
                <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
                  ...
                  <AlertCircle size={13} className="flex-shrink-0" />{authError}
                </motion.div>
              )}
            </AnimatePresence>
        ...
        <p className="text-center text-xs text-ink-400 mt-5">Your data is private and only visible to you.</p>
      </motion.div>
    </div>
  )
}
