...
const FREQ_OPTIONS  = [{ value: 'daily', label: 'Every day' }, { value: 'weekly', label: 'Times per week' }]
  ...
  return (
    <div className="relative" style={{ width: size, height: size }}>
      <svg width={size} height={size} style={{ transform: 'rotate(-90deg)' }}>
        <circle cx={size/2} cy={size/2} r={radius} fill="none" stroke={color} strokeWidth={strokeWidth} opacity={0.15} />
...
function HabitCard({ habit, checked, onToggle, streak, weeklyCount }) {
  ...
  return (
    <motion.button layout whileTap={{ scale: 0.93 }} onClick={() => onToggle(habit.id)}
      className={`flex flex-col items-center gap-2.5 p-4 rounded-2xl border transition-colors w-full ${
        checked || weeklyMet ? 'bg-surface-50 dark:bg-surface-800/60 border-surface-100 dark:border-surface-700'
      ...
      <ProgressRing progress={progress} size={76} strokeWidth={7} color={habit.color}>
        <motion.span className="text-2xl select-none"
          ...
          transition={{ duration: 0.3 }}>{habit.emoji}</motion.span>
      </ProgressRing>
      <div className="text-center">
        <p className="text-xs font-semibold text-ink-900 dark:text-ink-100 leading-tight line-clamp-2">{habit.name}</p>
        {isWeekly ? <p className="text-[10px] text-ink-400 mt-0.5">{weeklyCount}/{habit.weeklyTarget}× week</p>
          : streak > 0 ? <p className="text-[10px] text-ink-400 mt-0.5">🔥 {streak} day{streak !== 1 ? 's' : ''}</p> : null}
      </div>
    </motion.button>
  )
}
    ...
    <Modal open={open} onClose={onClose} title={initial ? 'Edit Habit' : 'Add Habit'}>
      <div className="space-y-4">
        <div className="flex justify-center py-1">
          <ProgressRing progress={0.6} size={72} strokeWidth={7} color={color}><span className="text-2xl">{emoji}</span></ProgressRing>
        </div>
        <div>
          ...
          <div className="flex flex-wrap gap-1.5 p-2.5 rounded-xl bg-surface-50 dark:bg-surface-900 max-h-24 overflow-y-auto">
            {EMOJI_OPTIONS.map(e => (
              <button key={e} onClick={() => setEmoji(e)}
                className={`text-lg p-1 rounded-lg transition-colors ${emoji === e ? 'bg-primary-100 dark:bg-primary-700/30 ring-1 ring-primary-400' : 'hover:bg-surface-100 dark:hover:bg-surface-700'}`}>{e}</button>
            ))}
          </div>
        </div>
        <div>
          <label className="block text-xs font-medium text-ink-400 mb-1">Habit name</label>
          <input value={name} onChange={e => setName(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleSave()} placeholder="e.g. Go to gym"
            ...
            {FREQ_OPTIONS.map(f => (
              <button key={f.value} onClick={() => setFrequency(f.value)}
                className={`flex-1 py-2 rounded-xl text-sm font-medium border transition-all ${
                  frequency === f.value ? 'bg-primary-50 dark:bg-primary-700/20 border-primary-400 text-primary-600 dark:text-primary-300'
        ...
        {frequency === 'weekly' && (
          <div>
            <label className="block text-xs font-medium text-ink-400 mb-2">Times per week: <span className="font-semibold" style={{ color }}>{target}</span></label>
            <div className="flex gap-1.5">
              {[1,2,3,4,5,6,7].map(n => (
                <button key={n} onClick={() => setTarget(n)}
                  className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${target === n ? 'text-white' : 'bg-surface-100 dark:bg-surface-700 text-ink-700 dark:text-ink-300 hover:bg-surface-200'}`}
                  style={target === n ? { backgroundColor: color } : {}}>{n}</button>
              ))}
            </div>
          </div>
        )}
        <div className="flex gap-2 pt-1">
          ...
          <Button className="flex-1" onClick={handleSave} disabled={!name.trim()}><Check size={14} /> Save</Button>
        </div>
      </div>
    </Modal>
  )
          ...
          {habits.map(habit => (
            <div key={habit.id} className="flex items-center gap-3 px-3 py-2.5 rounded-xl bg-surface-50 dark:bg-surface-900">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-ink-400">{habit.frequency === 'weekly' ? `${habit.weeklyTarget}× per week` : 'Daily'}</p>
              </div>
              <button onClick={() => setEditTarget(habit)} className="p-1.5 rounded-lg text-ink-400 hover:text-primary-500 transition-colors"><Edit2 size={13} /></button>
              <button onClick={() => onRemove(habit.id)} className="p-1.5 rounded-lg text-ink-400 hover:text-red-500 transition-colors"><Trash2 size={13} /></button>
            </div>
          ))}
          {habits.length < 10
            ? <Button variant="soft" className="w-full" onClick={() => setAddOpen(true)}><Plus size={14} /> Add habit</Button>
            : <p className="text-xs text-ink-400 text-center py-2">Maximum 10 habits reached</p>}
        </div>
      </Modal>
      <HabitFormModal open={addOpen} onClose={() => setAddOpen(false)} onSave={d => onAdd(d.name, d.emoji, d.frequency, d.weeklyTarget, d.color)} />
      {editTarget && <HabitFormModal open={!!editTarget} onClose={() => setEditTarget(null)} initial={editTarget} onSave={d => { onEdit(editTarget.id, d); setEditTarget(null) }} />}
    </>
  )
}
...
export function HabitTracker() {
  ...
  const handleToggle = (id) => {
    ...
    if (!wasChecked) { const h = habits.habits.find(h => h.id === id); awardAndToast('HABIT_CHECK', `Habit: ${h?.name}`) }
  }
  ...
  return (
    <PageWrapper>
      <div className="flex items-center justify-between mb-2">
        ...
        <Button variant="ghost" size="icon" onClick={() => setManageOpen(true)}><Settings size={18} /></Button>
      </div>
      {habits.totalHabits > 0 && <SummaryRing checked={checkedToday} total={habits.totalHabits} />}
      {habits.habits.length === 0 ? (
        <div className="py-12 text-center"><p className="text-4xl mb-3">🌱</p><p className="text-sm text-ink-400 mb-4">No habits yet.</p><Button onClick={() => setManageOpen(true)}><Plus size={15} /> Add habits</Button></div>
      ) : (
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
          <AnimatePresence>
            {habits.habits.map(habit => (
              <HabitCard key={habit.id} habit={habit} checked={habits.isCheckedToday(habit.id)}
                onToggle={handleToggle} streak={habits.getStreak(habit.id)} weeklyCount={habits.getWeeklyCount(habit.id)} />
            ))}
          </AnimatePresence>
        </div>
      )}
      <ManageModal open={manageOpen} onClose={() => setManageOpen(false)} habits={habits.habits}
        onAdd={habits.addHabit} onEdit={habits.editHabit} onRemove={habits.removeHabit} />
    </PageWrapper>
  )
}
