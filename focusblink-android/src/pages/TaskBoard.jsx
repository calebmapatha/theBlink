...
function DueDateBadge({ dueDate }) {
  ...
  if (due < today) return <span className="text-xs px-1.5 py-0.5 rounded-md text-red-500 bg-red-50 dark:bg-red-900/20">{label}</span>
  if (due.toDateString() === today.toDateString()) return <span className="text-xs px-1.5 py-0.5 rounded-md text-amber-600 bg-amber-50 dark:bg-amber-900/20">{label}</span>
  return <span className="text-xs px-1.5 py-0.5 rounded-md text-ink-400 bg-surface-100 dark:bg-surface-700">{label}</span>
}
  ...
  return (
    <motion.div layout draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}
      ...
      className={`flex items-center gap-3 p-3.5 rounded-xl border transition-colors ${
        leaving ? 'bg-success-100/40 dark:bg-success-500/10 border-success-200 dark:border-success-500/20'
      ...
      <GripVertical size={14} className="text-ink-300 flex-shrink-0 cursor-grab active:cursor-grabbing" />
      <Checkbox checked={leaving} onChange={handleComplete} />
      <span className={`flex-1 text-sm ${leaving ? 'line-through text-ink-400' : 'text-ink-900 dark:text-ink-100'}`}>{task.text}</span>
      {task.urgent && <Badge variant="urgent">urgent</Badge>}
      <DueDateBadge dueDate={task.dueDate} />
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => onToggleUrgent(task.id)} className={`p-1.5 rounded-lg transition-colors ${task.urgent ? 'text-warm-500' : 'text-ink-400 hover:text-warm-500'}`}>
...
function CompletedItem({ task, onDelete }) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl opacity-50">
      ...
      <button onClick={() => onDelete(task.id)} className="p-1 rounded text-ink-400 hover:text-red-400"><Trash2 size={13} /></button>
    </div>
  )
}
  ...
  return (
    <PageWrapper>
      ...
      <div className="mb-6">
        <div className="flex gap-2">
          <div className="flex-1 flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 focus-within:border-primary-400 focus-within:ring-1 focus-within:ring-primary-400 transition-all">
            ...
            <button onClick={() => setUrgent(u => !u)} className={`p-1 rounded transition-colors ${urgent ? 'text-warm-500' : 'text-ink-400 hover:text-warm-500'}`}>
              <Star size={15} fill={urgent ? 'currentColor' : 'none'} />
            </button>
            <button onClick={() => setShowDatePicker(v => !v)} className={`p-1 rounded transition-colors ${showDatePicker || dueDate ? 'text-primary-500' : 'text-ink-400 hover:text-primary-500'}`}>
      ...
      <div className="flex gap-1 mb-4 p-1 bg-surface-100 dark:bg-surface-800/60 rounded-xl">
        {[{ key: 'today', label: `Today (${tasks.todayTasks.length})` }, { key: 'backlog', label: `Backlog (${tasks.backlogTasks.length})` }].map(({ key, label }) => (
          <button key={key} onClick={() => setTab(key)}
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${
              tab === key ? 'bg-white dark:bg-surface-700 text-ink-900 dark:text-ink-100 shadow-sm' : 'text-ink-400 hover:text-ink-700 dark:hover:text-ink-100'
            }`}>{label}</button>
        ))}
      </div>
      <div className="space-y-2 group">
        <AnimatePresence mode="popLayout">
          {currentTasks.length === 0
            ? <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12 text-center"><p className="text-4xl mb-3">✨</p><p className="text-sm text-ink-400">{tab === 'today' ? 'Nothing here — add a task above!' : 'Backlog is clear'}</p></motion.div>
            : currentTasks.map(task => (
              <TaskItem key={task.id} task={task} onComplete={handleComplete} onDelete={tasks.deleteTask}
      ...
      {tab === 'today' && tasks.completedToday.length > 0 && (
        <div className="mt-6">
          ...
          <div className="space-y-1">{tasks.completedToday.map(task => <CompletedItem key={task.id} task={task} onDelete={tasks.deleteTask} />)}</div>
        </div>
      )}
    </PageWrapper>
  )
