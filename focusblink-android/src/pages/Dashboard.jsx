    ...
    <PageWrapper>
      <div className="mb-5">
        <div className="flex items-center gap-2 text-ink-400 text-xs mb-1">
          <Sun size={13} /><span>{formatDayHeader()}</span>
        </div>
      ...
      <motion.div variants={containerVariants} initial="hidden" animate="show" className="space-y-3">
        ...
        <motion.div variants={itemVariants}>
          <Card className="p-4 cursor-pointer hover:border-primary-300 dark:hover:border-primary-700 transition-colors" glow={isRunning} onClick={() => navigate('/timer')}>
            <div className="flex items-center gap-4">
              <div className="flex-1 min-w-0">
                <p className="text-xs font-semibold uppercase tracking-wider text-ink-400 mb-1">Focus Timer</p>
                <p className="text-3xl font-mono font-bold text-ink-900 dark:text-ink-100 tracking-tight leading-none">{formatSeconds(timer.secondsLeft)}</p>
                <div className="flex items-center gap-2 mt-2">
                  ...
                  {timer.sessionCount > 0 && <span className="text-xs text-ink-400">{timer.sessionCount} session{timer.sessionCount !== 1 ? 's' : ''} today</span>}
                </div>
              </div>
              {!isRunning && (
                <button onClick={e => { e.stopPropagation(); timer.start() }}
                  className="w-12 h-12 rounded-full bg-primary-500 hover:bg-primary-600 active:bg-primary-700 shadow-sm hover:shadow-glow-primary flex items-center justify-center transition-all flex-shrink-0">
                  <Play size={18} className="text-white ml-0.5" />
                </button>
              )}
              {isRunning && <Button size="sm" variant="soft" onClick={e => { e.stopPropagation(); navigate('/timer') }}>View</Button>}
            </div>
            {(isRunning || isPaused) && (
