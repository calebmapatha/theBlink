        ...
        {MOODS.map((emoji, i) => (
          <button key={i} onClick={() => onChange(i + 1)}
            className={`flex-1 flex flex-col items-center gap-1.5 py-3 rounded-xl border-2 transition-all ${
              value === i + 1 ? 'border-primary-400 bg-primary-50 dark:bg-primary-700/20'
  ...
  return (
    <div>
      ...
      <div className="flex gap-1.5">
        {[1,2,3,4,5].map(level => (
          <button key={level} onClick={() => onChange(level)} className="flex-1 flex flex-col items-center gap-2 py-3 rounded-xl transition-all">
            <div className="flex flex-col gap-0.5">
              {[5,4,3,2,1].map(bar => (
                <div key={bar} className={`w-6 rounded-sm transition-colors ${
                  bar <= level && value >= level ? 'bg-primary-500'
                  : bar <= level ? 'bg-primary-200 dark:bg-primary-700/40' : 'bg-surface-200 dark:bg-surface-700'
      ...
      {value > 0 && <p className="text-xs text-ink-400 mt-2 text-center">{ENERGY_LABELS[value - 1]}</p>}
    </div>
  )
}
  ...
  return (
    <Card className="p-5">
      <div className="flex items-center gap-2 mb-4">
        ...
        <button onClick={onEdit} className="ml-auto p-1.5 rounded-lg hover:bg-surface-100 dark:hover:bg-surface-700 text-ink-400"><Edit3 size={14} /></button>
      </div>
...
export function DailyCheckin() {
  ...
  const handleEdit = () => {
    if (checkin.todayCheckin) { setMood(checkin.todayCheckin.mood); setEnergy(checkin.todayCheckin.energy); setIntention(checkin.todayCheckin.intention || '') }
    setEditing(true)
  }
  ...
  return (
    <PageWrapper>
      ...
      {!checkin.todayCheckin || editing ? (
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="space-y-6">
          <Card className="p-5"><MoodPicker value={mood} onChange={setMood} /></Card>
          <Card className="p-5"><EnergyPicker value={energy} onChange={setEnergy} /></Card>
          <Card className="p-5">
            <label className="block text-sm font-medium text-ink-700 dark:text-ink-300 mb-2">One intention for today</label>
            <input value={intention} onChange={e => setIntention(e.target.value.slice(0, 120))}
          ...
          <div className="flex gap-2">
            {editing && <Button variant="ghost" onClick={() => setEditing(false)} className="flex-1">Cancel</Button>}
            <Button onClick={handleSubmit} className="flex-1"><CheckCircle size={16} />{editing ? 'Update' : 'Save Check-in'}</Button>
          </div>
        </motion.div>
      ) : (
