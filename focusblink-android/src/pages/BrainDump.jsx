    ...
    <PageWrapper>
      ...
      <div className="mb-8 rounded-2xl border border-surface-200 dark:border-surface-700 bg-white dark:bg-surface-800 overflow-hidden focus-within:border-primary-400 focus-within:ring-1 focus-within:ring-primary-400 transition-all">
        <textarea ref={textareaRef} value={text} onChange={e => setText(e.target.value)}
          onKeyDown={e => { if ((e.metaKey || e.ctrlKey) && e.key === 'Enter') handleDump() }}
          placeholder="What's on your mind? Racing thoughts, ideas, worries..."
          rows={5}
        ...
        <div className="flex items-center justify-between px-4 pb-3">
          <span className="text-xs text-ink-400">{text.length > 0 && `${text.length} chars · `}Cmd+Enter to dump</span>
          <Button onClick={handleDump} disabled={!text.trim()} size="sm"><Zap size={13} />Dump it</Button>
        </div>
      </div>
      {dump.entries.length === 0 ? (
        <div className="py-12 text-center"><p className="text-4xl mb-3">🧠</p><p className="text-sm text-ink-400">Your brain dump will appear here.</p></div>
      ) : (
        <div className="space-y-3">
          <p className="text-xs font-semibold uppercase tracking-wider text-ink-400">{dump.entries.length} entries</p>
          <AnimatePresence>
            {dump.entries.map(entry => (
              <motion.div key={entry.id} initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, height: 0 }} transition={{ duration: 0.2 }}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-ink-400 mb-2" title={formatTimestamp(entry.createdAt)}>{formatRelative(entry.createdAt)}</p>
                    <p className="text-sm text-ink-900 dark:text-ink-100 whitespace-pre-wrap leading-relaxed">{entry.text}</p>
                  </div>
                  <button onClick={() => dump.deleteEntry(entry.id)}
