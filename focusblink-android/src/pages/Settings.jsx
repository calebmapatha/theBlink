...
function Section({ title, children }) {
  return (
    <div className="mb-6">
      ...
      <Card className="divide-y divide-surface-100 dark:divide-surface-700 overflow-hidden p-0">{children}</Card>
    </div>
  )
}
...
function SettingsRow({ icon: Icon, label, value, onClick, danger }) {
  return (
    <button onClick={onClick} className={`w-full flex items-center gap-3 px-4 py-3.5 hover:bg-surface-50 dark:hover:bg-surface-700/50 transition-colors text-left ${danger ? 'text-red-500' : ''}`}>
  ...
  return (
    <Modal open={open} onClose={onClose} title="Edit Profile">
      <div className="space-y-4">
        <div className="flex justify-center"><div className="w-16 h-16 rounded-2xl bg-primary-100 dark:bg-primary-700/20 flex items-center justify-center text-4xl">{avatar}</div></div>
        <div>
          ...
          <div className="flex flex-wrap gap-2 p-2.5 rounded-xl bg-surface-50 dark:bg-surface-900 max-h-28 overflow-y-auto">
            {AVATAR_OPTIONS.map(e => (<button key={e} onClick={() => setAvatar(e)} className={`text-2xl p-1.5 rounded-xl transition-colors ${avatar === e ? 'bg-primary-100 dark:bg-primary-700/30 ring-1 ring-primary-400' : 'hover:bg-surface-100 dark:hover:bg-surface-700'}`}>{e}</button>))}
          </div>
        </div>
        <div>
        ...
        <div className="flex gap-2">
          ...
          <Button className="flex-1" onClick={() => { onSave({ displayName: name, avatarEmoji: avatar }); onClose() }}><Check size={14} /> Save</Button>
        </div>
      </div>
    </Modal>
  )
...
function ResetModal({ open, onClose, onConfirm }) {
  return (
    <Modal open={open} onClose={onClose} title="Reset to Defaults">
      <div className="space-y-4">
        <p className="text-sm text-ink-700 dark:text-ink-300">This will clear all tasks, habits, brain dumps, check-ins, and rewards — and restore the original default habits. Your profile and theme will be kept.</p>
        <div className="flex gap-2">
          <Button variant="ghost" className="flex-1" onClick={onClose}>Cancel</Button>
          <Button className="flex-1 bg-amber-500 hover:bg-amber-600" onClick={() => { onConfirm(); onClose() }}>Reset</Button>
        </div>
      </div>
    </Modal>
  )
...
function ClearDataModal({ open, onClose, onConfirm }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="Clear All Data">
      <div className="space-y-4">
        <p className="text-sm text-ink-700 dark:text-ink-300">This will permanently delete all your tasks, habits, brain dumps, check-ins, rewards, and profile. This cannot be undone.</p>
        <div>
        ...
        <div className="flex gap-2">
          <Button variant="ghost" className="flex-1" onClick={onClose}>Cancel</Button>
          <Button className="flex-1 bg-red-500 hover:bg-red-600" disabled={typed !== 'DELETE'} onClick={() => { onConfirm(); onClose() }}>Delete everything</Button>
        </div>
      </div>
    </Modal>
  )
...
export function Settings() {
  ...
  const handleResetDefaults = () => {
    ...
    Object.keys(localStorage).filter(k => k.startsWith(prefix) && !KEEP_KEYS.some(s => k.endsWith(s))).forEach(k => localStorage.removeItem(k))
  ...
  return (
    <PageWrapper>
      <div className="mb-6"><h1 className="text-2xl font-semibold text-ink-900 dark:text-ink-100">Settings</h1></div>
