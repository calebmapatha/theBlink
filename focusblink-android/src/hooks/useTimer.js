...
function timerReducer(state, action) {
  switch (action.type) {
    ...
    case 'RESET':  return { ...state, status: 'idle', secondsLeft: getModeSeconds(state.mode, action.settings) }
    ...
    case 'COMPLETE': {
      ...
      const nextMode = state.mode === 'work' ? (isLongBreak ? 'longBreak' : 'shortBreak') : 'work'
      return { mode: nextMode, status: 'idle', secondsLeft: getModeSeconds(nextMode, action.settings), sessionCount: newSessionCount }
    }
    case 'SET_MODE': return { ...state, mode: action.mode, status: 'idle', secondsLeft: getModeSeconds(action.mode, action.settings) }
...
export function useTimer(onSessionComplete) {
  ...
  const updateSettings = useCallback((s) => { setSettings(s); dispatch({ type: 'RESET', settings: s }) }, [setSettings])

  const totalSeconds = getModeSeconds(timerState.mode, settings)
  const progress     = 1 - timerState.secondsLeft / totalSeconds

  return { ...timerState, totalSeconds, progress, settings, start, pause, reset, skip, setMode, updateSettings }
}
