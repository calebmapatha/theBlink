...
export function useTasks(userId) {
  ...
  const completeTask  = useCallback((id) => setTasks(prev => prev.map(t => t.id === id ? { ...t, completedAt: new Date().toISOString() } : t)), [setTasks])
  const deleteTask    = useCallback((id) => setTasks(prev => prev.filter(t => t.id !== id)), [setTasks])
  const moveToToday   = useCallback((id) => setTasks(prev => prev.map(t => t.id === id ? { ...t, scheduledDate: todayKey() } : t)), [setTasks])
  const moveToBacklog = useCallback((id) => setTasks(prev => prev.map(t => t.id === id ? { ...t, scheduledDate: null } : t)), [setTasks])
  const toggleUrgent  = useCallback((id) => setTasks(prev => prev.map(t => t.id === id ? { ...t, urgent: !t.urgent } : t)), [setTasks])
