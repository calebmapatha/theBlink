...
function DueDateBadge({ dueDate }) {
  ...
  if (due < today) return <span className="text-xs px-1.5 py-0.5 rounded-md text-danger bg-red-50 dark:bg-red-900/20">{label}</span>
  ...
  return <span className="text-xs px-1.5 py-0.5 rounded-md text-faint bg-raised">{label}</span>
}
    ...
    <motion.div layout draggable onDragStart={onDragStart} onDragOver={onDragOver} onDrop={onDrop}
      ...
      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
        <button onClick={() => onToggleUrgent(task.id)}
          className={`focus-ring p-1.5 rounded-lg transition-colors ${task.urgent ? 'text-warm-500' : 'text-faint hover:text-warm-500'}`}>
          <Star size={14} fill={task.urgent ? 'currentColor' : 'none'} />
        </button>
        <button onClick={() => onMove(task.id)} className="focus-ring p-1.5 rounded-lg text-faint hover:text-accent transition-colors">
          {isToday ? <ArrowDown size={14} /> : <ArrowUp size={14} />}
        </button>
        <button onClick={() => onDelete(task.id)} className="focus-ring p-1.5 rounded-lg text-faint hover:text-danger transition-colors">
          <Trash2 size={14} />
        </button>
      </div>
    </motion.div>
...
function CompletedItem({ task, onDelete }) {
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl opacity-50">
      ...
      <button onClick={() => onDelete(task.id)} className="p-1 rounded text-faint hover:text-danger"><Trash2 size={13} /></button>
    </div>
  )
}
    ...
    <PageWrapper>
      ...
      <div className="mb-6">
        <div className="flex gap-2">
          <div className="flex-1 flex items-center gap-2 px-3.5 py-2.5 rounded-xl border border-line bg-surface focus-within:border-accent focus-within:ring-1 focus-within:ring-accent transition-all">
            ...
            <button onClick={() => setUrgent(u => !u)}
              className={`p-1 rounded transition-colors ${urgent ? 'text-warm-500' : 'text-faint hover:text-warm-500'}`}>
              <Star size={15} fill={urgent ? 'currentColor' : 'none'} />
            </button>
            <button onClick={() => setShowDatePicker(v => !v)}
              className={`p-1 rounded transition-colors ${showDatePicker || dueDate ? 'text-accent' : 'text-faint hover:text-accent'}`}>
      ...
      <div className="flex gap-1 mb-4 p-1 bg-raised/60 rounded-xl">
        {[
        ...
        <AnimatePresence mode="popLayout">
          {currentTasks.length === 0 ? (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="py-12 text-center">
              <div className="w-12 h-12 mx-auto mb-3 rounded-2xl bg-raised flex items-center justify-center"><Sparkles size={22} className="text-faint" /></div>
