      ...
      <div className="relative bg-white  border border-slate-200 shadow-2xl shadow-slate-300/40 p-5 space-y-4">
        ...
        <div className="space-y-2">
          {DIARY_ROWS.map(({ time, name, kind, status }) => (
            <div key={time} className="flex items-center gap-3  border border-slate-100 p-3">
              <div className="flex items-center gap-1.5 text-xs font-semibold text-primary-700 bg-primary-50  px-2 py-1 flex-shrink-0">
                <Clock size={11} />
                {time}
              </div>
              ...
              <span className={`text-[10px] font-semibold px-1.5 py-0.5  flex-shrink-0 ${
                status === 'Confirmed' ? 'bg-primary-500 text-white' : 'bg-slate-100 text-slate-500'
              }`}>
                {status}
              </span>
            </div>
        ...
        <div className="flex items-center justify-between  bg-slate-900 p-3.5">
          <div>
    ...
    <div className="h-[100dvh] overflow-y-auto bg-white text-slate-900">
      ...
      <header className="sticky top-0 z-40 bg-white/85 backdrop-blur-md border-b border-slate-100">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          <a href={home} className="flex items-center gap-2.5">
            <div className="w-9 h-9  bg-primary-500 flex items-center justify-center shadow-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <a href={startUrl} className="text-sm font-semibold bg-primary-500 hover:bg-primary-600 text-white px-4 py-2  transition-colors">
              List your practice
            </a>
          </div>
        </div>
      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-primary-50/80 via-white to-white">
        <div className="max-w-6xl mx-auto px-5 sm:px-8 pt-14 pb-16 lg:pt-20 lg:pb-20 grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          <div>
            <p className="text-sm font-bold text-primary-600 uppercase tracking-wide">For practitioners</p>
            ...
            <a href={startUrl}
              className="inline-flex items-center justify-center gap-2 mt-8 bg-primary-500 hover:bg-primary-600 active:bg-primary-700 text-white font-semibold px-6 py-3.5  transition-colors shadow-lg shadow-primary-500/25">
              List your practice
              <ArrowRight size={17} />
            </a>
          ...
          <div className="grid sm:grid-cols-2 gap-x-12 gap-y-10 mt-10">
            {POINTS.map(({ icon: Icon, title, desc }) => (
              <div key={title} className="flex items-start gap-4">
                <div className="w-11 h-11  bg-white border border-slate-200 shadow-sm flex items-center justify-center flex-shrink-0">
      ...
      <section className="max-w-6xl mx-auto px-5 sm:px-8 py-14 lg:py-20">
        <div className="relative overflow-hidden  bg-gradient-to-br from-primary-500 to-primary-700 text-white px-7 py-14 sm:px-14 text-center">
          <p className="text-sm font-bold text-primary-100 uppercase tracking-wide">Start with a 2-month free trial</p>
          ...
          <a href={startUrl}
            className="inline-flex items-center justify-center gap-2 mt-8 bg-white text-primary-700 hover:bg-primary-50 font-semibold px-7 py-3.5  transition-colors">
