...
function PhoneMock() {
  return (
    <div className="relative w-[290px] sm:w-[320px]">
      <div className="absolute -top-8 -left-10 w-40 h-40 bg-teal-200/50  blur-3xl" aria-hidden="true" />
      ...
      <div className="relative bg-white  border border-slate-200 shadow-2xl shadow-slate-300/40 p-5 space-y-4">
        ...
        <div className=" border border-teal-100 bg-teal-50/60 p-3.5">
          <div className="flex items-center justify-between mb-1.5">
            <p className="text-xs font-semibold text-teal-700 uppercase tracking-wide">Next appointment</p>
            <span className="text-[10px] font-semibold px-1.5 py-0.5  bg-teal-500 text-white">Confirmed</span>
          </div>
        ...
        <div className=" border border-slate-100 p-3.5">
          ...
          <div className="flex gap-1">
            {Array.from({ length: 12 }).map((_, i) => (
              <div key={i} className={`h-1.5 flex-1  ${i < 10 ? 'bg-teal-400' : 'bg-slate-200'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center gap-2  bg-slate-900 p-3.5">
          ...
          <div className="w-6 h-6  border-2 border-teal-400 border-t-transparent animate-spin [animation-duration:3s]" />
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div className="h-[100dvh] overflow-y-auto scroll-smooth bg-white text-slate-900">
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-8 h-16 flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <button
              ...
              className="text-sm font-semibold bg-teal-500 hover:bg-teal-600 text-white px-4 py-2  transition-colors"
            >
              Get started
            </button>
          </div>
        ...
        {menuOpen && (
          <div className="md:hidden border-t border-slate-100 bg-white px-5 py-4 space-y-1">
            ...
            <div className="flex gap-3 pt-3">
              ...
              <button onClick={() => onGetStarted('patient')} className="flex-1 py-2.5  bg-teal-500 text-white text-sm font-semibold">
                Get started
              </button>
            </div>
          </div>
        ...
        <div className="max-w-6xl mx-auto px-5 sm:px-8 pt-14 pb-16 lg:pt-20 lg:pb-24 grid lg:grid-cols-2 gap-12 lg:gap-8 items-center">
          <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-8">
              <button
                onClick={() => onGetStarted('patient')}
                className="inline-flex items-center justify-center gap-2 bg-teal-500 hover:bg-teal-600 active:bg-teal-700 text-white font-semibold px-6 py-3.5  transition-colors shadow-lg shadow-teal-500/25"
              >
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-12">
            {HOW_IT_WORKS.map(({ step, title, desc }) => (
              <motion.div key={step} {...fadeUp} className=" bg-slate-800/60 border border-slate-700/60 p-7">
                <div className="w-9 h-9  bg-teal-500 flex items-center justify-center text-sm font-bold mb-4">{step}</div>
          ...
          <div className="grid sm:grid-cols-2 gap-5 mt-12">
            {SECURITY_POINTS.map(({ icon: Icon, title, desc }) => (
              <motion.div key={title} {...fadeUp} className="flex items-start gap-4  bg-white border border-slate-100 p-6 shadow-sm">
                <div className="w-11 h-11  bg-teal-50 flex items-center justify-center flex-shrink-0">
      ...
      <section className="max-w-6xl mx-auto px-5 sm:px-8 py-16 lg:py-24">
        <motion.div
          {...fadeUp}
          className="relative overflow-hidden  bg-gradient-to-br from-teal-500 to-teal-700 text-white px-7 py-14 sm:px-14 text-center"
        >
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-3 mt-8">
            <button
              onClick={() => onGetStarted('patient')}
              className="bg-white text-teal-700 hover:bg-teal-50 font-semibold px-7 py-3.5  transition-colors"
            >
              Create your free account
            </button>
            <button
              onClick={() => onGetStarted('provider')}
              className="bg-teal-600/40 hover:bg-teal-600/60 border border-white/30 text-white font-semibold px-7 py-3.5  transition-colors"
            >
