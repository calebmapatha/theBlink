...
export function SignatureField({ onChange, className = '' }) {
  ...
  return (
    <div className={className}>
      <div className="relative rounded-xl border-2 border-dashed border-surface-300 dark:border-surface-600 overflow-hidden bg-white">
        <canvas ref={canvasRef} className="w-full h-32 touch-none block" />
        <span className="absolute bottom-1.5 left-3 text-[10px] text-surface-400 pointer-events-none select-none">
          Draw your signature here
        </span>
        <button type="button" onClick={clear}
          className="absolute top-1.5 right-1.5 p-1.5 rounded-lg bg-surface-100 text-surface-500 hover:text-red-500 hover:bg-red-50 transition-colors"
