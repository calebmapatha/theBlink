    ...
    <>
      <div className="sticky top-0 z-30 flex items-center justify-end px-4 sm:px-6 h-11 bg-bg/85 backdrop-blur-sm">
        <button onClick={() => setOpen(true)} aria-label="Notifications"
          className="relative p-2  text-muted hover:bg-raised transition-colors">
          <Bell size={18} />
          {unreadCount > 0 && (
            <span className="absolute top-0.5 right-0.5 min-w-[16px] h-4 px-1  bg-red-500 text-white text-[9px] font-bold flex items-center justify-center">
            ...
            {items.map(n => {
              ...
              return (
                <button key={n.id} onClick={() => handleClick(n)}
                  className={`w-full text-left flex gap-3 p-3  border transition-colors ${
                    wasUnread
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      {wasUnread && <span className="w-1.5 h-1.5  bg-accent flex-shrink-0" />}
                    </div>
