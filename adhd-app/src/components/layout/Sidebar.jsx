...
import { Timer, CheckSquare, Brain, Repeat, Sun, Trophy, Moon, Zap, LogOut } from 'lucide-react'
    ...
    <>
      ...
      <aside className="hidden md:flex flex-col w-56 flex-shrink-0 h-full bg-white dark:bg-surface-900 border-r border-surface-200 dark:border-surface-800">
        ...
        <div className="px-5 py-5 border-b border-surface-200 dark:border-surface-800">
          ...
          {user && (
            <p className="text-xs text-ink-400 mt-1.5 truncate">{user.displayName || user.email}</p>
          )}
        </div>
      ...
      {/* Mobile bottom bar */}
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-white dark:bg-surface-900 border-t border-surface-200 dark:border-surface-800 flex">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex-1 flex flex-col items-center gap-1 py-2 text-xs transition-colors ${
                isActive ? 'text-primary-500' : 'text-ink-400 hover:text-ink-700 dark:hover:text-ink-100'
              }`
            }
          >
            <Icon size={20} />
